...
void Producer(_int which)
{
  ...
  //   page 182 of the textbook.这个循环是通过调用ring->put（消息）生成N_MESSG消息以放入到环形缓冲区中。每条消息都携带一个消息id，该id由整数“num”表示。此消息id应放入插槽的“值”字段中。它还应该携带要存储在“thread_id”字段中的生产者线程的id，以便消费者线程可以知道哪个生产者稍后生成消息。您需要在调用环->放置（消息）前后放置同步代码。见教科书第182页的算法。
...
void Consumer(_int which)
{
  ...
  // this file.形成此使用者线程的输出文件名。此消费者接收到的所有消息都将记录在此文件中。
