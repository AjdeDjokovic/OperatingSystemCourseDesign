...
//	Save the CPU state of a user program on a context switch. 在上下文交换上保存用户程序的CPU状态。
//
//	Note that a user program thread has *two* sets of CPU registers --
//	one for its state while executing user code, one for its state
//	while executing kernel code.  This routine saves the former. 注意，一个用户程序线程有*两*组CPU寄存器——一组用于执行用户代码时的状态，一组用于执行内核代码时的状态
//----------------------------------------------------------------------

void Thread::SaveUserState()
{
    for (int i = 0; i < NumTotalRegs; i++)
        pcb->userRegisters[i] = machine->ReadRegister(i);
}

//----------------------------------------------------------------------
// Thread::RestoreUserState
//	Restore the CPU state of a user program on a context switch. 在上下文交换上恢复用户程序的CPU状态。
//
//	Note that a user program thread has *two* sets of CPU registers --
//	one for its state while executing user code, one for its state
//	while executing kernel code.  This routine restores the former. 注意，一个用户程序线程有*两*组CPU寄存器——一组用于执行用户代码时的状态，一组用于执行内核代码时的状态
//----------------------------------------------------------------------

void Thread::RestoreUserState()
{
    for (int i = 0; i < NumTotalRegs; i++)
        machine->WriteRegister(i, pcb->userRegisters[i]);
}
