 ...
 * go in the virtual address space. 这个程序读取一个COFF格式的文件，并输出一个NOFF格式的文件。NOFF格式实际上只是COFF文件的一个简单版本，记录每个段在NOFF文件中的位置，以及它在虚拟地址空间中的位置。
 * 
 ...
 * being NOPs when the host machine is little endian. 用于将单词和短单词转换为模拟机器的小尾数格式的例程。当主机是小endian时，这些结果是NOPs。
 */
