 ...
 * manual except for the following special ones: 操作码值。这些名称直接来自MIPS手册，但以下特殊名称除外：
 *
 * OP_UNIMP -		means that this instruction is legal, but hasn't
 *			been implemented in the simulator yet. 意味着此指令是合法的，但尚未在模拟器中实现。
 * OP_RES -		means that this is a reserved opcode (it isn't
 *			supported by the architecture). 意味着这是一个保留的操作码（架构不支持它）。
 */
...
/*
 * Miscellaneous definitions: 其他定义：
 */
...
// Stuff to help print out each instruction, for debugging 帮助打印每个指令的资料，用于调试
