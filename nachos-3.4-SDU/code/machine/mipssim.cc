...
//	Called by the kernel when the program starts up; never returns. 在Nachos上模拟用户级程序的执行。程序启动时由内核调用；从不返回。
//
//	This routine is re-entrant, in that it can be called multiple
//	times concurrently -- one for each thread executing user code.这个例程是可重入的，因为它可以同时调用多次——每个线程执行一个用户代码。
//----------------------------------------------------------------------
...
// 	at the instruction immediately after the syscall. 如果有任何异常或中断，我们调用异常处理程序，当它返回时，我们返回Run（），它将在循环中重新调用我们。这允许我们从头开始重新执行指令，以防我们的状态发生变化。在系统调用中，操作系统软件必须递增PC，因此在系统调用后立即从指令开始执行。
//
//	This routine is re-entrant, in that it can be called multiple
...
//	and the register set. 这个例程是可重入的，因为它可以同时调用多次——每个线程执行一个用户代码。我们从不缓存任何数据，从而获得重入性——每次调用时（或在异常或中断时捕获回Nachos内核后），我们总是从头开始模拟，并且在离开之前，我们总是将所有数据存储回机器寄存器和内存。这允许Nachos内核通过控制内存、翻译表和寄存器集的内容来控制我们的行为。
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
// Machine::DelayedLoad
// 	Simulate effects of a delayed load. 模拟延迟加载的效果。
//
// 	NOTE -- RaiseException/CheckInterrupts must also call DelayedLoad,
//	since any delayed load must get applied before we trap to the kernel. RaiseException/CheckInterrupts还必须调用delayed load，因为在捕获内核之前必须应用任何延迟的加载。
